...
namespace HITSBlazor.Utils.Mocks.Tests
{
    public static class MockTestAnswers
    {
        ...
        private static List<TestAnswer> GenerateBelbinTestAnswers(User user)
        {
            ...
            // Получаем вопросы 1 модуля
            var module1Questions = questions.Where(q => q.QuestionModuleNumber == 1).ToList();
            ...
            for (int i = 0; i < answersInModuleCount; ++i)
            {
                ...
                while (currentQuestion)
                {
                    ...
                    {
                        answersInModule[question.QuestionName] = $"{scores[i]}";
